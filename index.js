...
! function(t) {
	...
	async function a(t, e, r) {
		...
		let a = await g("index"),
			...
			p = (await l("SYSTEM_INDEX_LIST", !0))
			.slice(0, OPT.recentlySize);
		for (var h = 0; h < p.length; h++) p[h].createDate10 = p[h].createDate.substr(0, 10), p[h].url = "/article/" + p[h].id + "/" + p[h].link + ".html";
		let f = await i(e, r),
			d = f[0],
			w = f[1];
		for (h = 0; h < d.length; h++) d[h].createDate10 = d[h].createDate.substr(0, 10), d[h].createDateYear = d[h].createDate.substr(0, 4), d[h].createDateMonth = d[h].createDate.substr(5, 7), d[h].createDateDay = d[h].createDate.substr(8, 10), d[h].contentLength = d[h].contentText.length, d[h].url = "/article/" + d[h].id + "/" + d[h].link + ".html";
		let y = [{
			title: "上一页",
			url: "/" + t + "/" + e + "/" + (r - 1)
		}];
	...
	async function g(t) {
		return t = t.replace(".html", ""), (await fetch(OPT.themeURL + t + ".html", {
				cf: {
					cacheTtl: 600
				}
			}))
	...
	addEventListener("fetch", t => {
		t.respondWith(async function(t) {
			...
			}(t.request)) return new Response("Unauthorized", {
				headers: {
			...
			if ("/favicon.ico" == r.pathname) return new Response("404", {
				headers: {
					"content-type": "text/plain;charset=UTF-8"
				},
				status: 404
			...
			if ("sitemap.xml" == _) k = new Response(await async function() {
				...
				let e = await async function(t) {
					...
					if ("page" == r && parseInt(i) > 0) return await async function(t, e) {
						let r = await g("index"),
						...
						for (p = 0; p < h.length; p++) h[p].createDate10 = h[p].createDate.substr(0, 10), h[p].createDateYear = h[p].createDate.substr(0, 4), h[p].createDateMonth = h[p].createDate.substr(5, 7), h[p].createDateDay = h[p].createDate.substr(8, 10), h[p].contentLength = h[p].contentText.length, h[p].url = "/article/" + h[p].id + "/" + h[p].link + ".html";
						let f = [{
							title: "上一页",
							url: "/page/" + (e - 1)
						}];
						...
						let w = (e > 1 ? "page " + e + " - " : "") + OPT.siteName,
							y = OPT.keyWords,
							m = {};
						m.widgetMenuList = a, m.widgetCategoryList = i, m.widgetTagsList = s, m.widgetLinkList = o, m.widgetRecentlyList = u, m.articleList = h, m.pageNewer = f, m.pageOlder = d, m.title = w, m.keyWords = y;
					...
					if ("article" == r && i.length > 0) return await async function(t, e, r) {
						...
						let f = await s(e);
						for (h = 0; h < f.length; h++) f[h] && (f[h].createDate10 = f[h].createDate.substr(0, 10), f[h].contentLength = f[h].contentText.length, f[h].url = "/article/" + f[h].id + "/" + (void 0 === f[h].link ? "detail" : f[h].link) + ".html");
						let d = f[1];
						...
						let m = d.title + " - " + OPT.siteName,
							S = d.tags.concat(d.category)
							.join(","),
							T = {};
						T.widgetMenuList = i, T.widgetCategoryList = o, T.widgetTagsList = c, T.widgetLinkList = u, T.widgetRecentlyList = p, T.articleSingle = d, T.articleNewer = w, T.articleOlder = y, T.title = m, T.keyWords = S;
						let v = Object.assign({}, OPT);
						return v.password = "", v.user = "", v.cacheToken = "", v.cacheZoneId = "", T.OPT = v, n.render(a, T)
					}(0, i);
					if ("search" != r) return "admin" == r ? await async function(t, e) {
						new URL(t.url);
						...
						if ("publish" == e[1]) {
							...
							for (var r = 0; r < t.length; r++)
								if ("object" == typeof t[r].tags)
									for (var n = 0; n < t[r].tags.length; n++) - 1 == e.indexOf(t[r].tags[n]) && e.push(t[r].tags[n]);
						...
						if ("saveAddNew" == e[1]) {
							...
							if (r.length > 0 && i.length > 0 && s.length > 0 && h.length > 0 && d.length > 0) {
								y = await async function() {
										...
										return "" === t || null === t || "[]" === t || void 0 === t ? (await c("SYSTEM_INDEX_NUM", 1), "000001") : (await c("SYSTEM_INDEX_NUM", parseInt(t) + 1), ("00000" + (parseInt(t) + 1))
											.substr(-6))
									}(), w = d.replace(/<\/?[^>]*>/g, "")
									.trim()
	...
	t.exports = function() {
		...
		f.prototype.eos = function() {
			...
			var e, n, i, s = this.cache;
			if (s.hasOwnProperty(t)) e = s[t];
			else {
				for (var o, l, c, u = this, g = !1; u;) {
					if (t.indexOf(".") > 0)
						for (o = u.view, l = t.split("."), c = 0; null != o && c < l.length;) c === l.length - 1 && (g = a(o, l[c]) || (n = o, i = l[c], null != n && "object" != typeof n && n.hasOwnProperty && n.hasOwnProperty(i))), o = o[l[c++]];
			...
			return null == l && (l = function(t, r) {
				...
				for (var b, L, D, M, x, k, P = new f(t); !P.eos();) {
					if (b = P.pos, D = P.scanUntil(a))
						for (var U = 0, N = D.length; U < N; ++U) o(M = D.charAt(U)) ? (m.push(w.length), v += M) : (T = !0, l = !0, v += " "), w.push(["text", M, b, b + 1]), b += 1, "\n" === M && (E(), v = "", O = 0, l = !1);
					if (!P.scan(a)) break;
					if (S = !0, L = P.scan(h) || "name", P.scan(c), "=" === L ? (D = P.scanUntil(g), P.scan(g), P.scanUntil(i)) : "{" === L ? (D = P.scanUntil(s), P.scan(p), P.scanUntil(i), L = "&") : D = P.scanUntil(i), !P.scan(i)) throw new Error("Unclosed tag at " + P.pos);
				...
				return function(t) {
					for (var e, r = [], n = r, a = [], i = 0, s = t.length; i < s; ++i) switch ((e = t[i])[0]) {
						case "#":
						case "^":
			...
			for (var i, s, o, l = "", c = 0, u = t.length; c < u; ++c) o = void 0, "#" === (s = (i = t[c])[0]) ? o = this.renderSection(i, e, r, n, a) : "^" === s ? o = this.renderInverted(i, e, r, n, a) : ">" === s ? o = this.renderPartial(i, e, r, a) : "&" === s ? o = this.unescapedValue(i, e) : "name" === s ? o = this.escapedValue(i, e, a) : "text" === s && (o = this.rawValue(i)), void 0 !== o && (l += o);
			return l
			...
			if (c) {
				if (e(c))
					for (var u = 0, g = c.length; u < g; ++u) l += this.renderTokens(t[4], n.push(c[u]), a, i, s);
					...
					null != (c = c.call(n.view, i.slice(t[3], t[5]), (function(t) {
		...
		return y.clearCache = function() {
			...
			if ("string" != typeof t) throw new TypeError('Invalid template! Template should be a "string" but "' + (e(i = t) ? "array" : typeof i) + '" was given as the first argument for mustache#render(template, view, partials)');
			var i;
